    ...
    uint32_t pass_idx)
{
    ...
    for (const auto i : mesh_instances) {
        ...
        if (mesh.get_indices_size() > 0) {
            glDrawElements(GLenum(drawable.topology), mesh.get_indices_size(), GLenum(mesh.get_indices_type()), nullptr);
        } else {
            glDrawArrays(GLenum(drawable.topology), 0, mesh.get_vertices_size());
        }
