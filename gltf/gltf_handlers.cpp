...
namespace gltf
{
    ...
    accessor_data get_buffer_data(const tinygltf::Model& mdl, uint32_t accessor_idx)
    {
        return {
            ...
            mdl.buffers.at(mdl.bufferViews.at(mdl.accessors.at(accessor_idx).bufferView).buffer).data.data() + mdl.bufferViews.at(mdl.accessors.at(accessor_idx).bufferView).byteOffset,
    ...
    void load_meshes(const tinygltf::Model& mdl, const std::shared_ptr<scene_node>& root /*, const std::vector<animation>& anim */, gl::scene::scene& s)
    {
        ...
        const auto proj_matrix_idx = s.parameters.size() - 1;
        const auto view_matrix_idx = s.parameters.size() - 2;
        ...
        for (const auto& mesh : mdl.meshes) {
            for (const auto& primitive : mesh.primitives) {
...

                {
                    ...
                    indices_size = fill_buffer(ebo, mdl, primitive.indices);
                    vao.bind();
                    ebo.bind();
                    vao.unbind();
                    ebo.unbind();
                ...
                if (primitive.material >= 0) {
                    ...
                    s.parameters.emplace_back(gl::scene::parameter_type::f32, gl::scene::parameter_component_type::mat4);
                    s.parameters.emplace_back(gl::scene::parameter_type::f32, gl::scene::parameter_component_type::mat4);
                    s.parameters.emplace_back(gl::scene::parameter_type::i32, gl::scene::parameter_component_type::vec1); // anim key

                    m.add_parameter("u_PROJECTION", proj_matrix_idx);
                    m.add_parameter("u_VIEW", view_matrix_idx);
                    m.add_parameter("u_MODEL", s.parameters.size() - 3);
                    m.add_parameter("u_MVP", s.parameters.size() - 2);
                    m.add_parameter("u_ANIM_KEY", s.parameters.size() - 1);
                    ...
                    s.meshes.emplace_back(s.vertex_sources.size() - 1, gl::scene::mesh::indices_type(GL_UNSIGNED_INT), indices_size, v_count);
                    auto& n = s.drawables.emplace_back();
...
gl::scene::scene gltf::load_scene(const std::string& file_name, const std::string& env_tex_name, bool is_glb)
{
    ...
    gl::vertex_array_object cube_vao;
    {
        ...
        cube_vao.add_vertex_array(buf, 2, sizeof(float[3 + 3 + 2]), sizeof(float[3]));
    ...
    m.add_parameter("u_PROJECTION", 0);
    m.add_parameter("u_VIEW", 1);
    ...
    s.meshes.emplace_back(0, gl::scene::mesh::indices_type::none, 0, 36);
    auto& drawable = s.drawables.emplace_back();
    drawable.mesh_idx = 0;
    drawable.material_idx = 0;
    ...
    for (size_t i = 0; i < skins.size(); ++i) {
        for (auto& data : anims_matrices[i].animations) {
            gl::texture<GL_TEXTURE_2D> anim_tex{GL_NEAREST, GL_REPEAT};
